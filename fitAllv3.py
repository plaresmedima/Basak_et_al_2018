...
#Two different models are used for signal to concentration conversion- 1. linear and 2. nonlinear model. 
#Further, we have added correction due to arterial delay to the linear model and correction due to 
#measured tissue T1-values to the nonlinear model. 

# The 'model' variable refers to the model chosen for signal to concentration conversion and it could be 
#chosen as 'Linear' or 'Nonlinear'.
...
#'delay_condition': when 'True' the delay in the arrival of bolus from artery to the tissue of interest 
#is incorporated within the linear model using the function 'add_delay'.

#'T1_condition': when 'True' the measured tissue T1-values are used instead of the 
#literature T1-values within the nonlinear model . 
...
#'path_data': To be set as the path to the folder 'ROIdata'.
...
T1_condition = False#True  #  # When true measured T1-values are used in the nonlinear model
...
for d1 in study:
    ...
    ## Reading input files for measured T1-values ##
    Zt1 = np.genfromtxt('%s_T1.csv' %d1, dtype=None, delimiter=',',names=True, unpack = True) 
    patientT1 = np.array([x.decode() for x in Zt1['Patient']])
    ...
    for n in name:
        ...
        ## Measured T1-values corresponding to patient n ## 
        ind_T1 = np.where(patientT1 == '%s' %n)
        if T1_condition == True:
            try:
                if len(ind_T1[0]) == 2:
                    T10_kid = [Zt1['T1'][ind_T1[0][0]], Zt1['T1'][ind_T1[0][1]]]
                else:
                    T10_kid = [Zt1['T1'][ind_T1[0][0]], Zt1['T1'][ind_T1[0][0]]]
            except IndexError:
                T10_kid = [1/R10_kid, 1/R10_kid]
        else:
            T10_kid = [1/R10_kid, 1/R10_kid]
                ...
                if model == 'Linear':
                    if n=='AC 51a' and kidney_id[ii]== 'RK': # 2Cuptake model is used for one patient 
                        ...
                        if delay_condition == True:
                            res, fit = add_delay(P,Time, aif_new,roi_new,s,n,fit_model)      
                        else:
                    ...
                    roi_nonlinear = Concentration_SPGRESS(roi, roi0, T10_kid[ii], FA, TR, r)
                    if n=='AC 51a' and kidney_id[ii]== 'RK': # 2Cuptake model is used for one patient
...
#                plt.plot(Time,fit,'r')
    ...
    elif T1_condition == True:
        header = np.array(['Patient', 'Kidney', 'Parameter','Value_NLT1' ,'Access','AccessSF'])    
    else:
    ...
    if delay_condition == True:
        ...
        np.savetxt('%s_NLT1.csv' %(d1), ab1, fmt="%s",delimiter = ",")
    else:    
